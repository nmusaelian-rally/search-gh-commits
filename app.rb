...
def get_commits_data(owner, lookback)
...
post '/search' do
  redirect "/results/#{params[:owner]}/#{params[:lookback]}"
end

get '/results/:owner/:lookback' do
  commits_data = get_commits_data(params[:owner],params[:lookback])
